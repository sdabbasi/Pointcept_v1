...
class SemSegTester(TesterBase):
    def test(self):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            # scene_coords = np.zeros((segment.size, 3), dtype=np.float32)  # or torch.zeros if using torch
            # for fragment in fragment_list:
            #     partial_coords = fragment["coord"].cpu().numpy()  # (n, 3)
            #     partial_idx = fragment["index"].cpu().numpy()     # (n,)
            #     scene_coords[partial_idx] = partial_coords

            pred_save_path = os.path.join(save_path, "{}_pred.npy".format(data_name))
            # pred_save_path = False
            ...
            # CLASS_COLOR_20 = [SCANNET_COLOR_MAP_20[id] for id in pred]
            # colors = np.array(CLASS_COLOR_20, dtype=np.float32)/255.0

            # pcd = open3d.geometry.PointCloud()
            # pcd.points = open3d.utility.Vector3dVector(scene_coords)
            # pcd.colors = open3d.utility.Vector3dVector(colors)
            # open3d.io.write_point_cloud('tools/ply_vis/semseg.ply', pcd)
