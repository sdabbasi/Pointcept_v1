...
def process_single_sequence(sequence_file, save_path, sampled_interval, has_label=True, use_two_returns=True, update_info_only=False, get_seg_gts=False):
    ...
    for cnt, data in enumerate(dataset):
        ...
        if get_seg_gts and frame.lasers[0].ri_return1.segmentation_label_compressed:
            ret_outputs = frame_utils.parse_range_image_and_camera_projection(frame)
            if len(ret_outputs) == 4:
                range_images, camera_projections, seg_labels, range_image_top_pose = ret_outputs
                point_labels = get_point_cloud_labels(frame, range_images, seg_labels)
                point_labels_ri2 = get_point_cloud_labels(frame, range_images, seg_labels, ri_index=1)
                ...
                seg_labels_path = cur_save_dir / ('%04d.npy' % cnt)
                np.save(str(seg_labels_path).replace('.npy', '_labels.npy'), labels)
...
def get_point_cloud_labels(frame, range_images, segmentation_labels, ri_index=0):
    """
